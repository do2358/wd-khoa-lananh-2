...
const getStorage = (): StorageLike => (isBrowser ? window.localStorage : memoryStorage);
...
const LocalStorage = {
   ...
   */
  get<T = unknown>(key: string, path?: Path, defaultValue: T | null = null): T | null {
    ...
    try {
      ...
      for (const p of pathArray) {
        if (result == null || typeof result !== 'object' || !(p in result)) return defaultValue;
        result = result[p];
      }
      return result as T;
    } catch (e) {
  ...
  remove(key: string, path?: Path): void {
    ...
    try {
      ...
      for (let i = 0; i < pathArray.length - 1; i++) {
        const p = pathArray[i];
        if (cursor == null || typeof cursor !== 'object' || cursor[p] == null) return; // path not exist
        cursor = cursor[p];
      }
